...
This is the long-form Windows Driver Harness reference. Read it only when the
short `SKILL.md` asks for exact command snippets, registry details, debugger
launch recovery, or expanded troubleshooting.

Do not use this file to broaden discovery. It is not authorization to enumerate
VM folders, choose a VM, choose a snapshot, guess credentials, or continue past
a failed hard gate.
...
Resolve all relative paths below from the parent directory containing
`SKILL.md`. Call that directory `<SKILL_DIR>`.
...
- The user must provide a VMware snapshot saved after the guest has booted and
  classic WinDbg has successfully attached to the kernel target through
  VirtualKD/KDNET. A cold/offline Windows snapshot that has never reached a
  verified two-machine debugging state is not valid.
...
- During environment checks, create or update
  `windows-drv-harness.config.json` first and fill confirmed VM paths, tool
  paths, guest user, and guest password there. Plaintext credentials are
  allowed in this local gitignored config when the user asks for that workflow.
  Redact secrets in chat output and never commit the config.
...
- `guest.admin_user` and `guest.admin_password`: guest credentials used by
  `vmrun`/`vmware-mcp`; fill them into local config during environment checks
...
1. First inspect the MCP tools already exposed by the current agent. If
   `windbg-mcp` and `vmware-mcp` tools are callable, use them directly. If
   they are not visible, run `<SKILL_DIR>\scripts\detect-mcp.ps1`; if missing,
   run `<SKILL_DIR>\scripts\install-mcp.ps1` to prepare local tooling. Ask
   before running `<SKILL_DIR>\scripts\register-mcp.ps1 -Apply`. If
   registration fails or no supported client is found, run
   `<SKILL_DIR>\scripts\smoke-mcp-server.py` to verify the server command
   speaks MCP over stdio, show the manual commands, and stop until the user
   confirms their custom agent has registered them.
2. Create or update `<SKILL_DIR>\windows-drv-harness.config.json` first. If it
   is missing, create it from the example. Fill VMX path, snapshot name, guest
   admin user/password, and tool paths as the user provides or confirms them.
   Never discover VMX by drive scanning or by choosing from `vmrun list`. VMX
   path and snapshot name must be user-provided or explicitly user-confirmed.
3. If `guest.admin_password` is missing, ask for the guest password and write
   it to the local gitignored config unless the user explicitly prefers an
   `${env:VAR_NAME}` reference. Redact it in all output.
   Treat common guesses (`123456`, `12345678`, `admin`, `password`,
   `mypass`, `CHANGE_ME`, `TODO`, password equal to username) as missing
   unless the user explicitly confirms that exact value.
4. Check that the snapshot flag and debug transport flags match reality. The
   snapshot must already be VirtualKD/KDNET ready.
5. Verify VirtualKD registry and ensure exactly one vmmon in the same preflight.
...
10. Call `windbg-mcp.wm_session` until it reports an attached kernel target.
   ...
   If direct MCP tools are unavailable, use
   `<SKILL_DIR>\scripts\invoke-windbg-mcp.py` instead of writing a temporary
   client script in the repository root.
...
- Do not guess VM paths, snapshot names, guest usernames, passwords, or
  unusual tool paths.
- Do not continue after `vmrun` reports `Invalid user name or password for the
  guest OS`; stop and ask for corrected guest credentials. Do not brute-force
  common credentials, search git history for secrets, or dump environment
  variables.
...
- Do not create throwaway MCP helper scripts such as `call_mcp.py` in the
  repository root; use `scripts\invoke-windbg-mcp.py`.
