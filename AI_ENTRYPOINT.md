...
1. The user must provide a baseline VMware snapshot that already has guest-side
   ...
   is not valid.
2. `vmmon64.exe` must be running before the VM is restored or started, because
   it observes the VirtualKD event and auto-launches WinDbg.
